...
-- auto-scan by keyword (LocalPlayer incl PlayerGui/leaderstats + Character)
        ...
        if okA then
            for an, av in pairs(attrs) do
                if type(av) == "number" and looksLikeStamina(an) then
                    local t = {obj = obj, attr = an, max = av}
                    table.insert(staminaTargets, t); watch(t); freezeTarget(t)
                end
            end
        end
    end
...
local function openStaminaPicker()
    ...
    refreshBtn.Font = THEME.Font; refreshBtn.TextSize = 16; refreshBtn.ZIndex = 122; corner(refreshBtn, 6)
