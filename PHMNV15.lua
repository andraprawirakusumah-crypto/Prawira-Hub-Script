...
-- ============================================================
-- UNLIMITED STAMINA  (deteksi otomatis + kunci ke nilai maksimum)  (#3)
--   Stamina tiap game beda tempat/namanya, jadi ini BEST-EFFORT:
--   cari NumberValue/IntValue ATAU attribute angka yang namanya mirip
--   stamina/energy/sprint/breath/... di LocalPlayer (termasuk PlayerGui,
--   leaderstats, Backpack) + Character. Lalu "pin" ke nilai tertinggi yang
--   pernah terlihat. RINGAN: scan berat jarang (tiap ~3 dtk); pin sering tapi
--   cuma loop daftar target kecil (tiap ~0.15 dtk).
-- ============================================================
-- keyword diperluas: stamina/lari + napas + SENTER/flashlight/baterai + dll
local STAM_KEYWORDS = {
    "stamina","stam","energy","energi","sprint","run","dash","endur","fatigue","tired","lelah","capek",
    "breath","napas","nafas","oxygen","oksigen","lung",
    "senter","flashlight","flash","light","lamp","lampu","battery","baterai","batere","batre",
    "charge","fuel","bensin","power","juice","mana","gas",
}
local function looksLikeStamina(name)
    name = string.lower(name)
    for _, k in ipairs(STAM_KEYWORDS) do
...
local staminaConn
local staminaTargets = {}     -- auto  {obj, attr, max, conn}
local staminaManual = {}      -- pilihan user {obj, attr, max, conn}  (persist)
local staminaScanAcc, staminaPinAcc = 0, 0
local staminaSetToggle        -- forward ref tombol "Unlimited Stamina"

-- kunci 1 target ke nilai MAX yang pernah dilihat (value object / attribute)
local function freezeTarget(t)
    local o = t.obj
    if not (o and o.Parent) then return end
    if t.attr then
        local cur = o:GetAttribute(t.attr)
        if type(cur) == "number" then
            if cur > t.max then t.max = cur end
            if cur < t.max then pcall(function() o:SetAttribute(t.attr, t.max) end) end
        end
    else
        local ok, v = pcall(function() return o.Value end)
        if ok and type(v) == "number" then
            if v > t.max then t.max = v end
            if v < t.max then pcall(function() o.Value = t.max end) end
        end
    end
end

-- REAL-TIME: reset SEKETIKA saat nilai turun (lebih andal dari sekadar polling,
-- penting buat senter/stamina yang berkurang cepat). Event-driven = ringan.
local function unwatch(t)
...
local function watch(t)
    ...
    pcall(function()
        if t.attr then
            t.conn = o:GetAttributeChangedSignal(t.attr):Connect(function() freezeTarget(t) end)
        else
            t.conn = o.Changed:Connect(function() freezeTarget(t) end)
        end
    end)
end

-- auto-scan by keyword. Sekarang JAUH lebih dalam: NumberValue/IntValue +
-- atribut-angka di SEMUA descendant (Tool/Part/Folder, bukan cuma Humanoid)
-- pada LocalPlayer (+PlayerGui/Backpack) & Character, plus ReplicatedStorage.
local function scanStamina()
    for _, t in ipairs(staminaTargets) do unwatch(t) end
    staminaTargets = {}
    local function add(t) table.insert(staminaTargets, t); watch(t); freezeTarget(t) end
    local function checkAttrs(obj)
        local okA, attrs = pcall(function() return obj:GetAttributes() end)
        if okA then
            for an, av in pairs(attrs) do
                if type(av) == "number" and looksLikeStamina(an) then
                    add({obj = obj, attr = an, max = av})
                end
            end
        end
    end
    -- Player + Character: value-by-keyword + atribut-by-keyword di SEMUA descendant
    for _, rootObj in ipairs({ LocalPlayer, LocalPlayer.Character }) do
        if rootObj then
            checkAttrs(rootObj)
            local ok, desc = pcall(function() return rootObj:GetDescendants() end)
            if ok then
                for _, d in ipairs(desc) do
                    if (d:IsA("NumberValue") or d:IsA("IntValue")) and looksLikeStamina(d.Name) then
                        add({obj = d, max = d.Value})
                    end
                    checkAttrs(d)   -- atribut di Tool/Part/Folder/dll juga
                end
            end
        end
    end
    -- ReplicatedStorage: cukup NumberValue/IntValue by keyword (ringan)
    ...
    return #staminaTargets
end

local function pinAll()
    for _, t in ipairs(staminaTargets) do freezeTarget(t) end
    for _, t in ipairs(staminaManual) do freezeTarget(t) end
end

local function startStamina(silent)
    if staminaConn then return end
    local n = scanStamina()
    for _, t in ipairs(staminaManual) do watch(t); freezeTarget(t) end
    staminaScanAcc, staminaPinAcc = 0, 0
    if not silent then
        if (n + #staminaManual) == 0 then
            notify("Stamina auto tak ketemu — pakai 'Pilih Stamina Manual'", THEME.Yellow)
        else
            notify("Unlimited Stamina ON (" .. (n + #staminaManual) .. " sumber)", THEME.On)
        end
    end
    staminaConn = RunService.Heartbeat:Connect(function(dt)
        staminaPinAcc = staminaPinAcc + dt
        staminaScanAcc = staminaScanAcc + dt
        if staminaPinAcc >= 0.2 then staminaPinAcc = 0; pinAll() end   -- backup poll
        if staminaScanAcc >= 3 then staminaScanAcc = 0; scanStamina() end
    end)
end
...
local function stopStamina()
    ...
    for _, t in ipairs(staminaTargets) do unwatch(t) end
    for _, t in ipairs(staminaManual) do unwatch(t) end
    staminaTargets = {}
    -- staminaManual (pilihan user) SENGAJA disimpan; nyala lagi otomatis nanti
end

-- nyalakan stamina + sinkron tombol (dipakai dari picker manual)
local function ensureStaminaOn()
    if not state.stamina then
        state.stamina = true
        if staminaSetToggle then staminaSetToggle(true) end
    end
    startStamina(true)
end

-- ============================================================
-- PICKER MANUAL: pilih NumberValue/IntValue/atribut-angka utk di-FREEZE
--   Berguna kalau auto tak menebak (mis. bar "Senter", nama unik, dll).
-- ============================================================
local staminaPickerPanel
local function isManual(obj, attr)
    for i, t in ipairs(staminaManual) do
        if t.obj == obj and t.attr == attr then return i end
    end
    return nil
end
local STAM_PICK_CAP = 200   -- batas baris biar UI tak meledak di game besar
local function collectNumericSources()
    local out = {}
    local function addAttrs(obj)
        local okA, attrs = pcall(function() return obj:GetAttributes() end)
        if okA then
            for an, av in pairs(attrs) do
                if type(av) == "number" and #out < STAM_PICK_CAP then
                    table.insert(out, {obj = obj, attr = an, label = obj.Name .. " [" .. an .. "]", value = av})
                end
            end
        end
    end
    -- Player + Character: SEMUA NumberValue/IntValue + SEMUA atribut-angka di
    -- tiap descendant (Tool/Part/Folder). + ReplicatedStorage value-nya.
    local roots = { LocalPlayer, LocalPlayer.Character }
    local okRS, rs = pcall(function() return game:GetService("ReplicatedStorage") end)
    if okRS and rs then table.insert(roots, rs) end
    for ri, rootObj in ipairs(roots) do
        if rootObj and #out < STAM_PICK_CAP then
            if rootObj ~= (okRS and rs) then addAttrs(rootObj) end
            local ok, desc = pcall(function() return rootObj:GetDescendants() end)
            if ok then
                local isRS = (okRS and rootObj == rs)
                for _, d in ipairs(desc) do
                    if #out >= STAM_PICK_CAP then break end
                    if d:IsA("NumberValue") or d:IsA("IntValue") then
                        table.insert(out, {obj = d, label = d.Name, value = d.Value})
                    elseif not isRS then
                        addAttrs(d)   -- atribut angka di semua descendant Player/Char
                    end
                end
            end
        end
...
local function closeStaminaPicker()
    if staminaPickerPanel then
        local p = staminaPickerPanel; staminaPickerPanel = nil
...
local function openStaminaPicker()
    if staminaPickerPanel then return end
    local panel = Instance.new("Frame", ScreenGui)
    panel.Name = "StaminaPicker"
    panel.AnchorPoint = Vector2.new(0.5, 0.5)
    panel.Position = UDim2.new(0.5, 0, 0.5, 0)
    panel.Size = UDim2.fromOffset(300, 330)
    ...
    staminaPickerPanel = panel

    local hd = Instance.new("Frame", panel)
    hd.Size = UDim2.new(1, 0, 0, 36); hd.BackgroundColor3 = THEME.Panel; hd.BorderSizePixel = 0
    hd.ZIndex = 121; corner(hd, 12); gradient(hd, THEME.On, THEME.Blue, 0)
    local hlbl = Instance.new("TextLabel", hd)
    hlbl.Size = UDim2.new(1, -70, 1, 0); hlbl.Position = UDim2.new(0, 12, 0, 0)
    hlbl.BackgroundTransparency = 1; hlbl.Text = "PILIH STAMINA (FREEZE)"
    hlbl.TextColor3 = Color3.new(1, 1, 1); hlbl.Font = THEME.Font; hlbl.TextSize = 13
    hlbl.TextXAlignment = Enum.TextXAlignment.Left; hlbl.ZIndex = 122

    local refreshBtn = Instance.new("TextButton", hd)
    refreshBtn.Size = UDim2.new(0, 26, 0, 26); refreshBtn.Position = UDim2.new(1, -62, 0.5, -13)
    refreshBtn.BackgroundColor3 = THEME.Blue; refreshBtn.Text = "🔄"; refreshBtn.TextColor3 = Color3.new(1, 1, 1)
    refreshBtn.Font = THEME.Font; refreshBtn.TextSize = 14; refreshBtn.ZIndex = 122; corner(refreshBtn, 6)

    local cx = Instance.new("TextButton", hd)
    cx.Size = UDim2.new(0, 26, 0, 26); cx.Position = UDim2.new(1, -32, 0.5, -13)
    cx.BackgroundColor3 = THEME.Off; cx.Text = "X"; cx.TextColor3 = Color3.new(1, 1, 1)
    cx.Font = THEME.Font; cx.TextSize = 13; cx.ZIndex = 122; corner(cx, 6)
    cx.MouseButton1Click:Connect(closeStaminaPicker)

    local info = Instance.new("TextLabel", panel)
    info.Size = UDim2.new(1, -16, 0, 30); info.Position = UDim2.new(0, 8, 0, 40)
    info.BackgroundTransparency = 1; info.ZIndex = 121
    info.Text = "Tap nilai yang jadi stamina/senter -> FREEZE. Aktifkan saat bar PENUH. (🔄 refresh)"
    ...
    local listF = Instance.new("ScrollingFrame", panel)
    listF.Size = UDim2.new(1, -16, 1, -82); listF.Position = UDim2.new(0, 8, 0, 74)
    listF.BackgroundColor3 = Color3.fromRGB(12, 12, 18); listF.BorderSizePixel = 0
    listF.ScrollBarThickness = 5; listF.ScrollBarImageColor3 = THEME.Title
    listF.AutomaticCanvasSize = Enum.AutomaticSize.Y; listF.CanvasSize = UDim2.new(0, 0, 0, 0)
    listF.ZIndex = 121; corner(listF, 8)
    local lll = Instance.new("UIListLayout", listF); lll.Padding = UDim.new(0, 4); lll.SortOrder = Enum.SortOrder.LayoutOrder
    local llp = Instance.new("UIPadding", listF)
    llp.PaddingTop = UDim.new(0, 4); llp.PaddingBottom = UDim.new(0, 4)
    llp.PaddingLeft = UDim.new(0, 4); llp.PaddingRight = UDim.new(0, 6)

    local function rebuild()
        for _, c in ipairs(listF:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end
        local sources = collectNumericSources()
        if #sources == 0 then
            local none = Instance.new("TextLabel", listF)
            none.Size = UDim2.new(1, 0, 0, 28); none.BackgroundTransparency = 1
            none.Text = "(tak ada NumberValue/atribut angka di Player/Character)"
            none.TextColor3 = THEME.SubText; none.Font = THEME.FontReg; none.TextSize = 11; none.ZIndex = 122
            return
        end
        for i, src in ipairs(sources) do
            local row = Instance.new("Frame", listF)
            row.Size = UDim2.new(1, 0, 0, 30); row.BackgroundColor3 = THEME.Slot; row.BorderSizePixel = 0
            row.LayoutOrder = i; row.ZIndex = 122; corner(row, 6)
            local nm = Instance.new("TextLabel", row)
            nm.Size = UDim2.new(1, -78, 1, 0); nm.Position = UDim2.new(0, 8, 0, 0)
            nm.BackgroundTransparency = 1; nm.Text = src.label .. " = " .. tostring(src.value)
            nm.TextColor3 = THEME.Text; nm.Font = THEME.FontReg; nm.TextSize = 11
            nm.TextXAlignment = Enum.TextXAlignment.Left; nm.TextTruncate = Enum.TextTruncate.AtEnd; nm.ZIndex = 123
            local fb = Instance.new("TextButton", row)
            fb.Size = UDim2.new(0, 64, 0, 22); fb.Position = UDim2.new(1, -70, 0.5, -11)
            fb.Font = THEME.Font; fb.TextSize = 11; fb.TextColor3 = Color3.new(1, 1, 1); fb.ZIndex = 123; corner(fb, 6)
            local function syncBtn()
                if isManual(src.obj, src.attr) then
                    fb.Text = "FROZEN"; fb.BackgroundColor3 = THEME.On
                else
                    fb.Text = "FREEZE"; fb.BackgroundColor3 = THEME.Off
                end
            end
            syncBtn()
            fb.MouseButton1Click:Connect(function()
                local idx = isManual(src.obj, src.attr)
                if idx then
                    unwatch(staminaManual[idx]); table.remove(staminaManual, idx)
                    notify("Unfreeze: " .. src.label, THEME.Yellow)
                else
                    local cur = src.attr and src.obj:GetAttribute(src.attr) or src.obj.Value
                    local t = {obj = src.obj, attr = src.attr, max = (type(cur) == "number" and cur or 0)}
                    table.insert(staminaManual, t)
                    ensureStaminaOn(); watch(t); freezeTarget(t)
                    notify("Freeze: " .. src.label, THEME.On)
                end
                syncBtn()
            end)
        end
    ...
    -- drag panel picker (lewat header)
    do
...
staminaSetToggle = makeToggle(moveCard, "Unlimited Stamina", THEME.On, function(on)
    state.stamina = on
    if on then startStamina() else stopStamina() end
end)
-- tombol picker manual: kalau auto gagal nebak (mis. bar "Senter"), pilih sendiri
makeButton(moveCard, "🔍 Pilih Stamina Manual", THEME.Purple, function()
    openStaminaPicker()
end)
...
-- slider 2: JARAK keterangan dari karakter (radius cahaya)
makeSlider(brightCard, "Jarak Keterangan", brightRange, 6, 100, THEME.Orange, function(v)
    brightRange = v
    applyBrightness()
end)
...
local function cleanupAll()
    ...
    state.stamina = false; stopStamina()
    stopAutoPlay()
    stopBrightness()
...
-- (noclip/fly bind ulang part, cahaya brightness dibuat ulang, stamina rescan)
-- ============================================================
track(LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if state.stamina then scanStamina() end
end))
