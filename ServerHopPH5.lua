...
-- (SYNCED HISTORY: MANUAL JOIN + AUTO HOP)
...
-- ==========================================
local ConfigFile = "PrawiraHop_Config5.json"
local HistoryFile = "PrawiraHop_ServerHistory5.json"
local ServerHistory = {}
...
local function SaveHistory()
    pcall(function() 
        writefile(HistoryFile, S_H:JSONEncode(ServerHistory)) 
    end)
end
            ...
            if decoded and decoded.data then
                ...
                for _, serverData in ipairs(decoded.data) do
                    ...
                    if sPlayers < sMax then
                        ...
                        -- FUNGSI MANUAL JOIN (SYNCED HISTORY + UPDATE UI LANGSUNG)
                        JoinBtn.MouseButton1Click:Connect(function()
                            ...
                            -- CATAT HISTORY MESKIPUN MANUAL
                            ServerHistory[sId] = os.time()
                            SaveHistory()

                            -- UPDATE TAMPILAN SLOT KE COOLDOWN LANGSUNG
                            StatusLbl.Text = "Status: COOLDOWN (15m left)"
                            StatusLbl.TextColor3 = THEME.BtnStop
                            btnColor = THEME.Gray
                            JoinBtn.BackgroundColor3 = THEME.Gray
                            JoinBtn.Text = "WAIT..."
...
local function AutoHopLogic()
    ...
    local newServer, oldServer, currentTime = nil, nil, os.time()
    for _, v in pairs(Site.data) do
        local ID = tostring(v.id)
        if tonumber(v.playing) < tonumber(v.maxPlayers) then
            local lastJoined = ServerHistory[ID]
            if not lastJoined then
                if not newServer then newServer = ID; break end
            elseif (currentTime - lastJoined) >= 900 then
                if not oldServer then oldServer = ID end
            end
        end
    end
    if newServer then
        foundAnything = ""; backupOldServer = nil; ExecuteHop(newServer)
    else
        if oldServer and not backupOldServer then backupOldServer = oldServer end
        if Site.nextPageCursor and Site.nextPageCursor ~= "null" then
            foundAnything = Site.nextPageCursor
        else
            if backupOldServer then
                local target = backupOldServer; foundAnything = ""; backupOldServer = nil; ExecuteHop(target)
            else
                foundAnything = ""; backupOldServer = nil
            end
        end
    end
end
...
LoadServers()
if AutoStart then StartServerHop() end
