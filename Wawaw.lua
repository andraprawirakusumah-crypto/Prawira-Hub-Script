...
local function cari(namaWadah, nama, aksi)
    ...
    -- Mulai dari sini MAHAL. Dijatah sekali per 10 detik.
...
function AUTO.promptKasir()   return cari("Register",   "CheckoutPrompt", "Checkout") end
function AUTO.promptStaff()   return cari("Computer",   "StaffPrompt",    "Manage")   end
function AUTO.promptCraft()   return cari("CraftTable", "CraftSpot",      "Arrange")  end
...
do
    ...
    AUTO.set.panen = function(v)
        ...
        if v then
            startLoop("aPanen", function() instan(function() return doHarvestOnce(true) end) end,
                function() return config.autoDelay end)
    ...
    AUTO.set.tanam = function(v)
        ...
        if v then
            startLoop("aTanam", function()
                ...
                if PICU.tanam then return end
                PICU.tanam = true
                instan(function() return doPlantOnce(true) end)
                PICU.tanam = false
            end, function() return config.autoDelay end)
...
do
    ...
    task.spawn(function()
        while ScreenGui.Parent do
            ...
            -- auto sudah dimatikan, dan itu sumber lag yang paling besar.
            while ScreenGui.Parent and not tampil(AutoBody) do task.wait(0.4) end
            local pk = AUTO.promptKasir()
            local ps = AUTO.promptStaff()
            local pc = AUTO.promptCraft()
            local mode
            ...
            diagLbl.Text = table.concat({
                ...
                "  CheckoutPrompt : " .. (pk and ("ADA  (" .. (pk.Enabled and "siap" or "kosong") .. ")") or "TIDAK ADA"),
                "  StaffPrompt    : " .. (ps and "ADA" or "TIDAK ADA"),
                "  CraftSpot      : " .. (pc and "ADA" or "TIDAK ADA"),
                "",
