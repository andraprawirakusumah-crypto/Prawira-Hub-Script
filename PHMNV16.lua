...
local freeCamBtnDir = Vector3.zero        -- arah dari tombol on-screen (lokal)
local savedCamType
...
local function startFreeCam()
    ...
    freeCamBtnDir = Vector3.zero
    camera.CameraType = Enum.CameraType.Scriptable

    -- look: drag layar (Touch) / tahan klik-kanan (PC). Abaikan kalau di GUI.
    freeCamLookConns = {}
    local looking, lastPos = false, nil
    local SENS = 0.006
    table.insert(freeCamLookConns, UserInputService.InputBegan:Connect(function(i, gp)
        ...
        if i.UserInputType == Enum.UserInputType.Touch
           or i.UserInputType == Enum.UserInputType.MouseButton2 then
            looking = true; lastPos = i.Position
        end
    end))
    table.insert(freeCamLookConns, UserInputService.InputChanged:Connect(function(i)
        ...
        if i.UserInputType == Enum.UserInputType.MouseMovement then
            dx, dy = i.Delta.X, i.Delta.Y
        elseif i.UserInputType == Enum.UserInputType.Touch and lastPos then
    ...
    freeCamConn = RunService.RenderStepped:Connect(function(dt)
        ...
        local newCF = CFrame.new(freeCamPos) * rot
        camera.CFrame = newCF
    end)
end
...
local function bringCharacterToFreeCam()
    ...
    if hrp and freeCamPos then
        -- jatuhkan badan ke posisi kamera (orientasi tetap tegak)
        hrp.CFrame = CFrame.new(freeCamPos)
...
local function openFreeCamPanel()
    ...
    -- hold button helper -> set komponen freeCamBtnDir (sumbu lokal kamera)
    local function holdRow(order)
        local r = Instance.new("Frame", content)
        r.Size = UDim2.new(1, 0, 0, 32); r.BackgroundTransparency = 1; r.LayoutOrder = order
        local l = Instance.new("UIListLayout", r)
        l.FillDirection = Enum.FillDirection.Horizontal; l.Padding = UDim.new(0, 6)
        l.SortOrder = Enum.SortOrder.LayoutOrder
        return r
    end
    local function holdBtn(parent, text, col, axisFn, order)
        local b = Instance.new("TextButton", parent)
        b.Size = UDim2.new(0.5, -3, 1, 0); b.BackgroundColor3 = col; b.Text = text
        b.TextColor3 = Color3.new(1, 1, 1); b.Font = THEME.Font; b.TextSize = 13
        b.AutoButtonColor = false; b.LayoutOrder = order; b.ZIndex = 92; corner(b, 8)
        b.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                freeCamBtnDir = freeCamBtnDir + axisFn(); b.BackgroundTransparency = 0.35
            end
        end)
        local function rel(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                freeCamBtnDir = freeCamBtnDir - axisFn(); b.BackgroundTransparency = 0
            end
        end
        b.InputEnded:Connect(rel)
        return b
    end
    local r1 = holdRow(2)
    holdBtn(r1, "⬆ MAJU",  THEME.On,   function() return Vector3.new(0, 0, -1) end, 1)
    holdBtn(r1, "⬇ MUNDUR",THEME.Blue, function() return Vector3.new(0, 0, 1) end, 2)
    local r2 = holdRow(3)
    holdBtn(r2, "◀ KIRI",  THEME.Purple, function() return Vector3.new(-1, 0, 0) end, 1)
    holdBtn(r2, "▶ KANAN", THEME.Purple, function() return Vector3.new(1, 0, 0) end, 2)
    local r3 = holdRow(4)
    holdBtn(r3, "▲ NAIK",  THEME.Cyan, function() return Vector3.new(0, 1, 0) end, 1)
    holdBtn(r3, "▼ TURUN", THEME.Cyan, function() return Vector3.new(0, -1, 0) end, 2)

    local bring = Instance.new("TextButton", content)
    bring.Size = UDim2.new(1, 0, 0, 30); bring.BackgroundColor3 = THEME.Yellow
    bring.Text = "📌 Bring Character ke sini"; bring.TextColor3 = Color3.new(0, 0, 0)
    bring.Font = THEME.Font; bring.TextSize = 12; bring.LayoutOrder = 5; bring.ZIndex = 92; corner(bring, 8)
