...
-- Tombol Minimize & X: UKURAN SAMA PERSIS (kotak 30x30, TextSize 15, corner 8),
-- diberi jarak 20px supaya tidak salah pencet. Glyph "–" (dash tengah) dipakai
-- biar terlihat sejajar/center dengan "X" (bukan "_" yang nempel ke bawah).
local BTN_SIZE = UDim2.new(0, 30, 0, 30)
local BTN_TEXTSIZE = 15

local MinBtn = Instance.new("TextButton", Header)
MinBtn.Size = BTN_SIZE
MinBtn.Position = UDim2.new(1, -90, 0.5, -15)   -- digeser ke kiri (jarak ke X = 20px)
...
Body.Position = UDim2.new(0, 12, 0, 90)
...
OutputBody.Size = UDim2.new(1, -24, 1, -102)
OutputBody.Position = UDim2.new(0, 12, 0, 90)
...
ServerBody.Size = UDim2.new(1, -24, 1, -102)
ServerBody.Position = UDim2.new(0, 12, 0, 90)
...
-- ============================================================
local STAM_KEYWORDS = {
    "stamina", "stam", "energy", "sprint", "breath", "endur",
    "fatigue", "oxygen", "stamna", "tired",
}
...
local staminaConn
local staminaTargets = {}   -- {obj=, attr=nil|name, max=number}
local staminaScanAcc, staminaPinAcc = 0, 0

-- RINGAN: hanya scan LocalPlayer (sudah termasuk PlayerGui, leaderstats,
-- Backpack) + Character. Tidak menyentuh ReplicatedStorage (sering ribuan
-- objek & server-authoritative). GetAttributes hanya dipanggil untuk Humanoid,
-- Model, dan root -> jauh lebih hemat dari memanggilnya di tiap descendant.
local function scanStamina()
        ...
        if okA then
            for an, av in pairs(attrs) do
                if type(av) == "number" and looksLikeStamina(an) then
                    table.insert(staminaTargets, {obj = obj, attr = an, max = av})
                end
            end
        end
    end
    ...
    for _, rootObj in ipairs(roots) do
        if rootObj and not seen[rootObj] then
            seen[rootObj] = true
            checkAttrs(rootObj)
            ...
            if ok then
                for _, d in ipairs(desc) do
                    if (d:IsA("NumberValue") or d:IsA("IntValue")) and looksLikeStamina(d.Name) then
                        table.insert(staminaTargets, {obj = d, max = d.Value})
                    elseif d:IsA("Humanoid") or d:IsA("Model") then
                        checkAttrs(d)
                    end
                end
            end
...
local function pinStamina()
    for _, t in ipairs(staminaTargets) do
        local o = t.obj
        if o and o.Parent then
            if t.attr then
                ...
                if type(cur) == "number" then
                    ...
                    if cur < t.max then o:SetAttribute(t.attr, t.max) end
                end
            else
                if o.Value > t.max then t.max = o.Value end
                if o.Value < t.max then o.Value = t.max end
            end
        end
    end
end
...
local function startStamina()
    ...
    if n == 0 then
        notify("Stamina tak terdeteksi otomatis di game ini", THEME.Yellow)
    else
        notify("Unlimited Stamina ON (" .. n .. " sumber)", THEME.On)
    end
    staminaConn = RunService.Heartbeat:Connect(function(dt)
        staminaPinAcc = staminaPinAcc + dt
        staminaScanAcc = staminaScanAcc + dt
        if staminaPinAcc >= 0.15 then
            staminaPinAcc = 0
            pinStamina()
        end
        if staminaScanAcc >= 3 then   -- scan ulang berkala (game tambah value baru)
            staminaScanAcc = 0
            scanStamina()
        end
    end)
...
makeToggle(moveCard, "Unlimited Stamina", THEME.On, function(on)
    state.stamina = on
