...
-- circle drag + click-to-restore (robust untuk HP: lacak input object spesifik)
do
    ...
    -- pakai input.Changed dari objek input itu sendiri (paling andal di mobile)
    ...
    Circle.InputEnded:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
            if dragInput == i or dragInput == nil then
                dragInput = nil
...
local function startSpeed()
    ...
    speedConn = RunService.Heartbeat:Connect(function()
        -- re-fetch humanoid kalau referensi basi (mis. respawn)
        local char = LocalPlayer.Character
        local h = char and char:FindFirstChildOfClass("Humanoid")
        if h then
            humanoid = h
            if h.WalkSpeed ~= config.walkSpeed then
